import { Visualization } from "./manager";
...
export class UI {
    ...
    private residueLabel!: Selection<HTMLDivElement, unknown, null, undefined>; //label used to display ligand ids
    private tooltip!: Selection<HTMLDivElement, unknown, null, undefined>; // mouseover tooltips
    ...
    public register(p: Config.UIParameters) {
        let toolbar: Selection<HTMLDivElement, unknown, null, undefined> | undefined = undefined;
        let dynamicPanel: Selection<HTMLDivElement, unknown, null, undefined> | undefined = undefined;

        if (p.menu) {
            toolbar = select(this.parent)
                .append('div')
            ...
            // create a fast custom tooltip (appears instantly)
            const quickTooltip = select('body')
                .append("div")
            ...
            if (p.help) {
                ...
                let cont = select(this.parent).append('div').attr('id', 'pdb-lig-env-help-container')
            ...
            // attach fast hover tooltips to all icons inside dynamic panel
            dynamicPanel.selectAll("i")
                .on("mouseenter", function (event) {
                    const title = select(this).attr("about");
                    if (!title) return;
                ...
                .on("mouseleave", function () {
                    quickTooltip.style("opacity", "0");
                });
        }

        if (p.tooltip) {
            this.tooltip = select(this.parent).append('div')
        ...
        if (p.residueLabel) {
            this.residueLabel = select(this.parent).append('div')
                .classed('pdb-lig-env-label', true)
    ...
    private changeHelp(showResidues: boolean) {
        let ligHelpBtn = select(this.parent).select('#pdb-lig-env-help-residues-btn');
        let bondHelpBtn = select(this.parent).select('#pdb-lig-env-help-bonds-btn');
        let bondHelpSection = select(this.parent).select('#pdb-lig-env-help-bonds');
        let ligHelpSection = select(this.parent).select('#pdb-lig-env-help-ligands');
    ...
    private showNames() {
        let btn = select(this.parent).select('#pdb-lig-env-names-btn');
    ...
    private showHelp() {
        let el = select(this.parent).select('#pdb-lig-env-help-container');
        let btn = select(this.parent).select('#pdb-lig-env-help-btn');
    ...
    private displayToolbarPanel(show: boolean) {
        let dynPanel = select(this.parent).select('#pdb-lig-env-menu-dynamic-panel');
        let el = select(this.parent).select('#pdb-lig-env-help-container');
     ...
     */
    private fullScreen() {
        let btn = select(this.parent).select('#pdb-lig-env-fullscreen-btn');
    ...
    private nodeMouseEnterEventHandler(e: any) {
        if (!this.tooltip) return; // safeguard
    ...
    private showLigandLabel(e: any) {
        if (!this.residueLabel) return; // safeguard
