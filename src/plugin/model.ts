...
export type Environments = 0 | 1 | 2;
...
export class InteractionNode implements SimulationNodeDatum {
    id: string;
    residue: Residue;
    ...
    constructor(r: Residue, scale: number, id: string, x: number | undefined = undefined, y: number | undefined = undefined) {
        this.residue = r;
        this.id = id;
...
export class ResidueResidueLink extends Link {
    ...
    public isBoundMoleculeLink(): boolean {
        return (
            ...
            this.interactions.get(InteractionType.AtomAtom)?.includes('covalent') === true
        );
    }
...
export class LigandResidueLink extends Link {
    ...
    public toTooltip() {
        ...
        this.interaction.forEach(x => {
            ...
            if (!targetAbbreviation) console.warn("undefined targetAbbreviation in toTooltip()");
...
export class Gradient {
     ...
     */
    public getScales(){
        const schemeKey = `scheme${this.colorScheme}` as keyof typeof d3Chromatic;
        const color = d3Chromatic[schemeKey]?.[9] ?? ['#ccc'];
        const weightMax = Number(max(this.weight));
        const gradient = {
            firstScale: new Scale(
                scaleLinear([0, weightMax], [20, 30]),
                scaleLinear([0,weightMax], ["#FFFFFF", color[4]])
            ),
            secondScale: new Scale(
                scaleLinear([0,weightMax], [10, 20]),
                scaleLinear([0, weightMax], [color[5], color[7]])
            ),
            thirdScale: new Scale(
                scaleLinear([0, weightMax], [2, 10]),
                scaleLinear([0, weightMax], [color[8], color[9]])
            )
            };
